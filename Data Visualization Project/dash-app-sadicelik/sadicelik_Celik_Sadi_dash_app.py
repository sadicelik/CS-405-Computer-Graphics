# Dash - NYC Taxi Data Application
...
mapbox_access_token = "pk.eyJ1Ijoic2FkaWNlbGxpIiwiYSI6ImNra2M5OXhsbDAzbDQyb3A3bzhud2F5bWoifQ.EkLdQqMa9lWWTE60bALVJA"
...
datetime_df = df.drop(columns=["VendorID", "passenger_count", "trip_distance", "RatecodeID", 
                 "store_and_fwd_flag", "DOLocationID", "payment_type", "fare_amount", 
                 "extra", "mta_tax", "tip_amount", "tolls_amount", "improvement_surcharge", "total_amount"])
datetime_df["tpep_pickup_datetime"] = pd.to_datetime(datetime_df["tpep_pickup_datetime"], format="%Y-%m-%d %H:%M")
datetime_df["tpep_dropoff_datetime"] = pd.to_datetime(datetime_df["tpep_dropoff_datetime"], format="%Y-%m-%d %H:%M")
...
totalList = []
for day in merged_df.groupby(merged_df.index.day):
    totalList.append(day[1])
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.H2("DASH - NYC TAXI DATA APPLICATION"),
                        html.P(
                        ...
                        html.Div(
                            ...
                            children=[
                                dcc.DatePickerSingle(
                                    ...
                                    max_date_allowed=dt(2017, 1, 31),
                                    initial_visible_month=dt(2017, 1, 1),
                                    date=dt(2017, 1, 1).date(),
                        ...
                        dcc.Markdown(
                            children=[
                                "Source: [NYC Taxi Data](https://www1.nyc.gov/site/tlc/about/tlc-trip-record-data.page)"
                            ]
                        ),
                    ],
                ),
                ...
                html.Div(
                    ...
                    children=[
                        ...
                        html.Div(
                            ...
                            children=[
                                "Select any of the bars on the histogram to section taxi data by time."
                            ],
                        ),
...
def get_selection(day, selection):
    xVal = []
    yVal = []
    xSelected = []
    colorVal = [
    ...
    for i in range(24):
        ...
        yVal.append(len(totalList[day][totalList[day].index.hour == i]))
...
def update_total_rides(datePicked):
    ...
    return "Total Number of rides: {:,d}".format(
        len(totalList[date_picked.day - 1])
    )
    ...
    if selection != None or len(selection) != 0:
        ...
        for x in selection:
            totalInSelection += len(
                totalList[date_picked.day - 1][
                    totalList[date_picked.day - 1].index.hour
                    == int(x)
                ]
            )
...
def update_histogram(datePicked, selection):
    ...
    [xVal, yVal, colorVal] = get_selection(dayPicked, selection)
...
def getLatLonColor(selectedData, day):
    listCoords = totalList[day]
    ...
    for time in selectedData:
        if selectedData.index(time) is not len(selectedData) - 1:
            listStr += "(totalList[day].index.hour==" + str(int(time)) + ") | "
        else:
            listStr += "(totalList[day].index.hour==" + str(int(time)) + ")]"
    return eval(listStr)
...
def update_graph(datePicked, selectedData, selectedLocation):
    ...
    date_picked = dt.strptime(datePicked, "%Y-%m-%d")
    dayPicked = date_picked.day - 1
    listCoords = getLatLonColor(selectedData, dayPicked)
